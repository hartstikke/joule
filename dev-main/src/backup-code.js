...
function main() {
  ...
  const submitForm = async (e) => {
    ...
    try {
      ...
      let calculatePercentage = percentages[term] / 100 || 0
      ...
      if (financingMethod === 'BONUS') {
        ...
        monthlyLeasePriceNumber = r.leasePrice.monthlyLeasePrice * 1.21 * 12
      } else if (financingMethod === 'YEAR_END_PREMIUM') {
        advantagePerMonthNumber = (beforeEmployerCostTotal - afterEmployerCostTotal) / 12
        document.querySelectorAll('[data-bonus="show"]').forEach((el) => el.classList.add('hide'))
        document.querySelectorAll('[data-bonus="hide"]').forEach((el) => el.classList.remove('hide'))
        monthlyLeasePriceNumber = r.leasePrice.monthlyLeasePrice * 1.21
      } else {
        advantagePerMonthNumber = (beforeEmployerCostTotal - afterEmployerCostTotal) / 12
        document.querySelectorAll('[data-bonus="show"]').forEach((el) => el.classList.add('hide'))
        document.querySelectorAll('[data-bonus="hide"]').forEach((el) => el.classList.remove('hide'))
