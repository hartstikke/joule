  ...
  updateCalculationResults(r, data, calculatePercentage) {
    ...
    switch (data.financingMethod) {
      case 'BONUS':
        ...
        document.querySelectorAll('[data-yearend="hide"],[data-yearend="show"]').forEach((el) => el.classList.add('hide'))
        break
      case 'YEAR_END_PREMIUM':
        ...
        document.querySelectorAll('[data-bonus="hide"], [data-bonus="show"]').forEach((el) => el.classList.add('hide'))
        break
      default:
        monthlyLeasePriceNumber = r.leasePrice.monthlyLeasePrice * VAT_RATE
        // advantagePerMonthNumber = (beforeEmployerCostTotal - afterEmployerCostTotal) / 12
        advantagePerMonthNumber = monthlyLeasePriceNumber + r.diffs.grossSalary
        document.querySelectorAll('[data-yearend="show"], [data-bonus="show"]').forEach((el) => el.classList.add('hide'))
        document.querySelectorAll('[data-yearend="hide"], [data-bonus="hide"]').forEach((el) => el.classList.remove('hide'))
    }

    // Update calculated values
    this.updateCalculatedValues(r, data, beforeEmployerCostTotal, afterEmployerCostTotal, monthlyLeasePriceNumber, advantagePerMonthNumber, calculatePercentage)
  }

  updateCalculatedValues(r, data, beforeEmployerCostTotal, afterEmployerCostTotal, monthlyLeasePriceNumber, advantagePerMonthNumber, calculatePercentage) {
    ...
    this.setValue('beforeSocialSecurity', this.formatNegativeAmount(r.beforeLeaseSalary.employerCosts.socialSecurityStructuralReduction), false)
    this.setValue('afterSocialSecurity', this.formatNegativeAmount(r.afterLeaseSalary.employerCosts.socialSecurityStructuralReduction), false)

    // Other expenses
    this.setValue('beforeOtherExpenses', this.formatNegativeAmount(r.beforeLeaseSalary.employerCosts.otherExpenses), false)
    this.setValue('afterOtherExpenses', this.formatNegativeAmount(r.afterLeaseSalary.employerCosts.otherExpenses), false)

    // Advantages and totals
    this.setValue('advantage', advantagePerMonthNumber.toFixed(1), true, true)
    // this.setValue('recoup', (monthlyLeasePriceNumber - advantagePerMonthNumber).toFixed(1), true)
    this.setValue('recoup', Math.abs(r.diffs.grossSalary).toFixed(1), true, true)
    ...
    this.setValue('savedTotalYearly', `${(advantagePerMonthNumber * 12).toFixed(1)}`)
    ...
    // Net calculations
    this.setValue('monthlyNet', Math.abs(r.diffs.monthlyNet.toFixed(2)))
