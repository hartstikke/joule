...
const submitForm = async (e) => {
    ...
    let bikeAllowancePrice = parseFloat(document.querySelector('#bikeAllowanceInput').value)
    console.log(bikeAllowancePrice)

    // data to send to OFFR
    const data = {
    ...
    try {
        ...
        /**
         * Set the results from OFFR into HTML
         */

        const $potentialMonthlyBikeAllowance = document.querySelector('[data-result="potentialMonthlyBikeAllowance"]')
        const $monthlyLeasePrice = document.querySelector('[data-result="monthlyLeasePrice"]')
        const $maintenanceCost = document.querySelector('[data-result="maintenanceCost"]')

        $monthlyLeasePrice.innerText = (result.leasePrice.monthlyLeasePrice * 1.21).toFixed(2)
        $maintenanceCost.innerText = '€ ' + (result.leasePrice.maintenanceCost * 12 * 1.21).toFixed(2)
        ...
        document.querySelector('[data-result="monthlyNet"]').innerText = '€ ' + Math.abs(result.diffs.monthlyNet.toFixed(2))
        document.querySelector('[data-result="annualNet"]').innerText = '€ ' + Math.abs(result.diffs.annualNet.toFixed(2))
        ...
        let savedValue =
            Math.abs(result.leasePrice.totalLeasePrice * 1.21) - Math.abs(result.diffs.totalNet) - Math.abs(data.computeLeaseRateDto.bikeRetailPrice * data.computeLeaseRateDto.residualValue)
        ...
        $potentialMonthlyBikeAllowance.innerText = '€ ' + Math.min((4 * (data.commuteKilometers * 2) * data.commuteWeeklyFrequency * bikeAllowancePrice).toFixed(2), 625)
